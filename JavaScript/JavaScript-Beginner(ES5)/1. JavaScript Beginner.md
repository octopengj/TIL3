...
### 1.3. 스크립팅 언어 특징, 컴파일 순서, JS와 객체지향
...
- new Array( )
  ...
  - 배열 생성 기준
    ...
    - 작성한 순서로 엘리먼트에 설정
      ...
      console.log(two); // 0: [10, 20]
      ```
    
    - 파라미터에 숫자를 작성하면 숫자만큼 엘리먼트 생성 
...
#### 6.8.4. 엘리먼트 추가/삭제 메커니즘, delete 연산자
...
- 배열 엘리먼트 삭제 메커니즘
  ...
  초창기에는 성능문제로 앞으로 하나씩 당겨서 엘리먼트를 이동하면 처리 시간이 걸리기 때문에 이런 메커니즘으로 작동한다. 현재는 삭제하는 것이 가능.
...
- | 구분   | 데이터(값)                |
  ...
  - 배열의 일부를 복사하여 배열로 반환, 원본은 바뀌지 않음
    ...
    - 첫번째 파라미터만 작성하면 두번째 파라미터는 디폴트: length
    
    - 첫번째 파라미터 값이 두번째 파라미터 값보다 클 때 빈배열 반환
    
    - 파라미터에 음수 작성하면 length값을 더해서 실행
      ...
      console.log(value.slice(4, 3)); //[ ]
      
      console.log(value.slice(-4, -2)); // [2, 3]
      ```
      
      -4 + 5 = 1 / -2 + 5 = 3 ==> slice(1, 3)이 됨

#### 6.8.7. 엘리먼트 값을 문자열로 변환
...
- pop( )
  ...
  - 삭제한엘리먼트 값이 undefined로 남지 않고 완전히 삭제됨
    
    length 값이 하나 줄어 듬
    
    ```javascript
    var value = [1, 2, 3, 4];
    console.log(value.pop()); // 4
...
- | 구분   | 데이터(값)         |
  ...
  - 삭제한 위치에 세번째 파라미터를 삽입
    
    ```javascript
    var value = [1, 2, 3, 4, 5];
    console.log(value.splice(1, 3, "나", "다", "라")); // [2, 3, 4]
    console.log(value); // [1, "나", "다", "라", 5]
    ```
  
  - 파라미터를 작성하지 않으면 빈배열 반환
...
### 6.9. Array 오브젝트(ES5 기준)

#### 6.9.1. 프로퍼티 리스트, 함수 여부 체크
...
| reduce( )       | 콜백 함수에서 반환 값을 파라미터 값으로 사용         |
| reduceRight( )  | reduce( )와 같음. 배열의 끝에서 앞으로 진행     |
...
- indexOf( )
  ...
  - 파라미터 값과 같은 엘리먼트의 인덱스 반환
    ...
    - String과 Array의 indexOf( ) 차이
      ...
      ```javascript
      console.log("abcabc".indexOf("c", -2)); // 2
      ```
      
      두번째 파라미터를 음수로 작성을 하면 0으로 간주하여 처음부터 검색
      
      <u>Array</u>
      
      ```javascript
      var list = ["a", "b", "c", "a", "b", "c"];
      console.log(list.indexOf("c", -2)); // 5
      ```
      
      두번째 파라미터를 음수로 작성을 하면 음수에 length값을 더해서 시작 인덱스 적용 -2 + 6 = 4
...
                리스트가 여러개일 경우 분리하지 않으면 코드 중복이 되기 때문에 분리하여 

                사용하는 것이 좋다.

- break, continue 사용 불가 / throw 사용 가능
...
- for( )와 forEach( )
  ...
  - forEach( )는 처음부터 끝까지 반복, 반복 중간에 끝나지 않음
  
  - for( )는 반복여부를 함수 코드를 읽어야 알 수 있음
    
    - break, continue를 만나면 변경됨
...
- every( )
  ...
  - 배열의 엘리먼트를 하나씩 읽어가면서 false를 반환할 때까지 콜백 함수 호출
...
- | 구분   | 데이터(값) |
  ...
  - 인스턴스를 생성하지 않고 true또는 false로 변환
    
    ```javascript
    var value = [1, "1", "0", "false"];
    for (var i = 0; i < value.length; i++){
        console.log(Boolean(value[i]) + 1);  
    };
    /*
    2
    2
...
### 6.11. Object 오브젝트(ES5 기준)

#### 6.11.1. ES5 Object 특징, ES5 Object 함수, 프로퍼티 디스크립터
...
- 프로퍼티 디스크립터
  ...
  | enumerable   | [[Enumeralbe]], 프로퍼티 열거 가능 여부   |
  | configurable | [[Configurable]], 프로퍼티 삭제 가능 여부 |

#### 6.11.2. Object 프로퍼티 추가
...
| access | get          | Function Object, undefined | undefined | 프로퍼티 함수               |
...
- keys( )
  ...
  - 열거 가능 프로퍼티 이름 반환
    ...
    enumerable이 true인 것만 반환

#### 6.11.6. 프로퍼티 디스크립터 함수
...
- getOwnPropertyDescriptor( )
  ...
  - 프로퍼티 디스크립터의 속성 이름, 값 반환
    ...
    Object.defineProperty(obj, "book", {
        value: "책",
        writable: true, enumerabale: true
    });
    var desc = 
